...
## BLAST Tools And Network

B1 `blast_sequence` is not a Primer3-style local binary. It calls the NCBI
BLAST URL API over the network and needs `NCBI_BLAST_EMAIL` plus outbound HTTPS
to `https://blast.ncbi.nlm.nih.gov` from wherever the MCP server runs. Inside
WSL this works like any other HTTPS client: no extra install, no API key. B2
