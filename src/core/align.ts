 ...
 *
 * Deterministic dynamic-programming alignment. The core is pure: it takes two
 * sequence strings plus scoring parameters and returns the optimal global
 ...
 * Needleman-Wunsch, not affine). Default scoring: match +1, mismatch -1,
 * gap -2. Traceback prefers a diagonal (match/mismatch) step, then a gap in
 * the target, then a gap in the query, so identical inputs always produce
 * identical output. No uppercase normalization, no ambiguity codes, no
 * heuristics, no network: the caller passes uppercase sequences if it wants
...
export function alignSequences(
  ...
  return {
    ...
    gaps,
    score: score[m][n],
    scoringParams: { match, mismatch, gap },
  };
}
